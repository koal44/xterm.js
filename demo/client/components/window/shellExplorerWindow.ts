 ...
 *
 * Start an interactive shell and select an appropriate profile.
 * Inject a test grapheme, then punch HOME and END.
 * Infer column width from the editor's motion or repaint behavior.
 */
...
interface ISocketReporter { log: string[] /* on: boolean; */ }
const REPORTER = new WeakMap<WebSocket, ISocketReporter>();
...
export class ShellExplorerWindow extends BaseWindow {
  ...
  public build(container: HTMLElement): void {
    ...
    addRow(root, 'test cases: ',
      ...TESTS.map(t => mkButton(t, `Inject "${formatCodePoints(t)}"`, () => this._inject((this._useTestPrefix ? TEST_PREFIX : '') + t)))
    );
  ...
  private _tryInstallReporter(): void {
    ...
    if (s) this._rep = this._installSocketReporter(s);
  }
  ...
  private _inject(data: string): void {
    ...
    if (!trigger) {this._appendOut('[dev] term not ready for triggerDataEvent\n'); return; }
  ...
  private _installSocketReporter(socket: WebSocket): ISocketReporter {
    ...
    socket.send = ((data: any) => {
      this._logSock(rep, '<<< ', data);
      return origSend(data);
    }) as any;

    socket.addEventListener('message', (ev: MessageEvent) => {
      this._logSock(rep, '>>> ', ev.data);
    });
  ...
  private _previewBinary(data: ArrayBufferLike | ArrayBufferView): string {
...
function mkCheckbox(label: string, title: string, checked: boolean, onChange: (v: boolean) => void): HTMLLabelElement {
...
function formatCodePoints(s: string): string {
  ...
  for (const ch of s) {
    const cp = ch.codePointAt(0)!;
    cps.push(`{${cp.toString(16).padStart(4, '0')}}`);
  }
  return cps.join('');
}
